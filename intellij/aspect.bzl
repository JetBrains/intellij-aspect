...
def _merge_target_info(builder, target, ctx):
    ...
    for name, module_provider in intellij_provider.MODULE_MAP.items():
        ...
        extra_key_aspects += getattr(provider.internal_value, "aspect_ids", [])
    ...
    intellij_info_builder.append_ide_infos(builder, [ide_info.write(target, ctx, info, extra_key_aspects)])
