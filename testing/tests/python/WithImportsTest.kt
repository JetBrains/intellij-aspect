...
class WithImportsTest {
  ...
  fun testLib() {
    ...
    assertThat(
      target.pythonTargetInfo.generatedSourcesList.map {
        it.relativePath
      },
  ...
  fun testPureLib() {
    ...
    assertThat(target.srcsList.map { it.relativePath }).containsExactly("src/lib/foo/__init__.py", "src/lib/foo/foo.py")
  }
}
